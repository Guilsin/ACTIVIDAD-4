...
public class MainUsuario extends JFrame implements
ActionListener {
	private Container contenedor;private JButton cilindro, esfera, pirámide;
	...
	public void actionPerformed(ActionEvent evento) {
		if (evento.getSource() == esfera) {
			EsferaUsuario esfera = new EsferaUsuario();
			esfera.setVisible(true);
		}
		if (evento.getSource() == cilindro) {
			CilindroUsuario cilindro = new CilindroUsuario();
			cilindro.setVisible(true);
		}
		if (evento.getSource() == pirámide) {
			PiramideUsuario pirámide = new PiramideUsuario();
